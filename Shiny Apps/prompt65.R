...
server <- function(input, output, session) {
  ...
  current_data <- reactive({
    if (!is.null(rv$edited_data)) {
      ...
      return(rv$original_data)
    }
  })
  
  output$table <- renderDataTable({
    req(current_data())
    datatable(current_data(), editable = 'cell', rownames = FALSE, options = list(stateSave = TRUE))
  })
  ...
  observeEvent(input$impute, {
    ...
    if (!is.null(column) && !is.null(strategy)) {
      ...
      rv$edited_data <- imputed_data
      replaceData(proxy, rv$edited_data, resetPaging = FALSE, clearSelection = TRUE)
    }
  })
