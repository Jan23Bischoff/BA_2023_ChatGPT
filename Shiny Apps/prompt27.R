    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Regression",
                 ...
                 selectInput("independent_var", "Independent Variable:", choices = NULL),
...
server <- function(input, output, session) {
  ...
  run_regression <- eventReactive(input$run_regression, {
    ...
    summary(lm(formula, data = dataset()))
  })
  ...
  run_prediction <- eventReactive(input$run_prediction, {
    ...
    predict(run_regression(), newdata = prediction_dataset())
  })
  
  # Display prediction output
  output$prediction_output <- renderTable({
    req(run_prediction())
    data.frame(Prediction = run_prediction())
  })
